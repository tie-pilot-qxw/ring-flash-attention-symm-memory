...
from .utils import RingCommSymmDirect, RingComm, update_out_and_lse, get_default_args
...
def ring_flash_attn_symm_direct_forward(
    process_group,
    q: torch.Tensor, 
    k: torch.Tensor, # must be symmetric memory
    v: torch.Tensor, # must be symmetric memory
    k_hdl: _SymmetricMemory,
    v_hdl: _SymmetricMemory,
    softmax_scale,
    dropout_p=0,
    causal=True,
    window_size=(-1, -1),
    ...
    for step in range(comm.world_size):
        with torch.cuda.stream(comm.stream[comm.stream_id]):
            if step == 0:
                k, v = k, v
            else:
                k, v = comm.send_recv_kv(step)
...
class RingFlashAttnSymmDirectFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        out, softmax_lse = ring_flash_attn_symm_direct_forward(
            group,
            q,
            k,
            v,
...
def ring_flash_attn_symm_direct_func(
    q,
    k,
    v,
    k_hdl,
    ...
    return RingFlashAttnSymmDirectFunc.apply(
        q,
        k,
        v,
        k_hdl,
